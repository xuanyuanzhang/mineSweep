...
public class Grid extends Button {
    ...
    private boolean isClicked;// whether clicked. Nothing will happen to click on a clicked grid.
    ...
    public boolean click() {
        ...
        if(isBomb) {
            ...
            this.setLabel(Integer.toString(this.getNearbyBombCount())); // Currently not accessible.
            return false;
        }
    }
    ...
    public int getNearbyBombCount() {
        int count = 0;
        if(this.northWest != null && this.northWest.isBomb) count++;
        if(this.north != null && this.north.isBomb) count++;
        if(this.northEast != null && this.northEast.isBomb) count++;
        if(this.east != null && this.east.isBomb) count++;
        if(this.southEast != null && this.southEast.isBomb) count++;
        if(this.south != null && this.south.isBomb) count++;
        if(this.southWest != null && this.southWest.isBomb) count++;
        if(this.west != null && this.west.isBomb) count++;
    ...
    private void isFreeSpace(Set<Grid> set, Grid nearbyGrid){
    	if(nearbyGrid != null && nearbyGrid.isBomb == false) {
    		nearbyGrid.click();
            if(nearbyGrid.getNearbyBombCount() == 0) {
    ...
    public void setChange(boolean isClicked) {
